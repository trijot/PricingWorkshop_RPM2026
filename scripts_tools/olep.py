...
olp.loc[:,'eff_dt'] = pd.to_datetime(olp.eff_dt).dt.normalize() #set effective dates as datetimes
olp.loc[:,'exp_dt'] = olp.eff_dt + pd.DateOffset(years=1) #expirations for 1-yr policies
olp.loc[:,'total_days'] = (olp.exp_dt - olp.eff_dt).dt.days 
